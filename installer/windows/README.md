...
La instalación inicial, las actualizaciones con datos, la reanudación tras una cancelación causada por McAfee y el reinicio solicitado desde Opciones se comprobaron en Windows 11. La espera del proceso antes del respaldo pasó en una actualización repetida con `pass13`. El 26 de septiembre se forzaron en este Windows dos fallos **después de copiar archivos**: uno tras validar la versión nueva y otro después de arrancarla. En ambos se recuperaron la interfaz, el binario y SQLite; la base anterior y posterior coincidió por SHA-256 y Setup devolvió 20 con la corrección. Una actualización normal `pass15` terminó con 0 y conservó los datos. El arranque después de reiniciar con `pass15` también pasó: servidor sano, base íntegra y catálogo conservado. Quedan los clics en Edge y el acceso desde otro dispositivo en la red de destino. La prueba del otro Windows quedó fuera del alcance acordado para esta revisión local. El estado está en [Pruebas pendientes de Windows](../../docs/PRUEBAS_PENDIENTES_WINDOWS.md); el historial detallado está en `docs/REVISION_INSTALACION_STACK_2026-09-25.md`.

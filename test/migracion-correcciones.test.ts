...
describe("migración 011 sobre una base que ya aplicó 010", () => {
  ...
  it("no toca ninguna otra tabla salvo los marcadores de la migración legacy posterior", () => {
    ...
    const antes = db
      .prepare(
        "SELECT name, sql FROM sqlite_master WHERE type = 'table' AND name NOT IN ('precuentas', 'caja_handoffs', 'cuentas', 'ordenes', 'contorno_grupos', 'contorno_variantes', 'plato_slots', 'plato_slot_grupos', 'orden_linea_contornos') ORDER BY name",
      )
      .all();

...
La lista actual de pruebas no realizadas, evidencia exigida y colaboración manual necesaria está en [Pruebas pendientes de Windows](PRUEBAS_PENDIENTES_WINDOWS.md). La configuración local ya indica `approval_policy = "never"` y `sandbox_mode = "danger-full-access"`; el rechazo se produjo en el ejecutor de Codex antes de iniciar PowerShell y no identificó una regla editable ni un error de Windows. No se ha vuelto a intentar la misma operación bloqueada.

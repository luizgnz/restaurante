...
La revisión automática volvió a bloquear la compilación local de un Setup con migración intencionalmente inválida (`blocked by policy`), sin indicar un motivo más específico. Esa prueba no se ejecutó en este equipo. El commit final quedó local: el conector GitHub rechazó escribir el árbol del repositorio con HTTP 403 (`Resource not accessible by integration`), el MCP alternativo devolvió 404 y `git push` sin interacción informó que no hay credencial disponible. La PR #19 permanece pendiente de actualizar hasta reautorizar el acceso de escritura; por ello el nuevo workflow de recuperación tampoco se ha ejecutado en CI.

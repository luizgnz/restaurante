...
Para la validación pendiente en el otro Windows se preparó en Documentos `Restaurante-Setup-0.1.0-ui-clean-pass12-x64.zip` (SHA-256 `422495BD4A5DF42896B07770A02397B0A76CAE4F3FDF553092924F3AA32B1598`). Contiene el `.exe`, los dos `.bin`, `LEEME-INSTALACION.txt` y `SHA256SUMS.txt`. Se verificó la integridad del ZIP y que las cuatro sumas internas coinciden con sus archivos. Este empaquetado no demuestra que el Setup funcione en el otro equipo; allí se debe extraer completo y ejecutar el `.exe` junto a los `.bin`.

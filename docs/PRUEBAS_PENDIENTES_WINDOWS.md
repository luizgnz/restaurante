...
| --- | --- | --- |
| Arranque tras reiniciar Windows con `pass15` | El reinicio anterior correspondía a una versión previa; `pass15` se instaló y arrancó sano, pero aún no se ha reiniciado este Windows desde entonces. El token no elevado de esta sesión recibe «Access is denied» al consultar la tarea con `schtasks`; el proceso `restaurante.exe` sí atiende el puerto 8080 en la sesión de servicios. | Tras reiniciar: consultar la tarea `Restaurante POS` con privilegios de administrador, comprobar `/api/salud`, interfaz disponible y mismo dato ID 9. |

...
- [ ] Corregir en #7 el formato `$8900` del diálogo de cancelación, propagarlo a #8 y repetir pruebas/revisión visual.
- [ ] Integrar en `feat/nucleo-pos-v1` la cadena #6 → #7 → #8 → #9 → #10 → #11 → #12 tras corregir el salón móvil y completar las revisiones. Seguir `docs/PLAN_MERGE_PRS_6_12.md` y conservar `prototype/ui-responsive` como referencia del PR original.
- [ ] 2026-09-06: corregir el solapamiento de mesas del salón en móvil: una regla posterior de `.mesa-odoo` impone `position: absolute` sobre el layout móvil. Evaluar el aislamiento de estilos y verificar con capturas actuales a 390/768/1280 px antes de aprobar la parte visual; la división de PRs no corrige este fallo.
